...
# 新增：区分任务类型
MULTI_LABEL_DATASETS = ('PPI', 'deezer-europe')  # 多标签数据集
SINGLE_LABEL_DATASETS = ('cora', 'citeseer', 'pubmed', 'airport', 'disease', 'node2vec_PPI','Mashup_PPI','alzheimers','Clin_Term_COOC','diabet','diabetuci','jiaolvyiyu')  # 单标签分类数据集
...
if len(dataset.label.shape) > 1:
    ...
    num_class = int(dataset.label.max().item() + 1)
...
elif args.dataset in SINGLE_LABEL_DATASETS:
...
for run in range(args.runs):
    ...
    if args.dataset in ['cora', 'citeseer', 'pubmed', 'airport', 'disease',"PPI"] and args.protocol == 'semi':
...
for run in range(args.runs):
    ...
    # Handle PPI dataset separately
    if args.dataset in ['cora', 'citeseer', 'pubmed', 'airport', 'disease',"PPI","node2vec_PPI"] and args.protocol == 'semi':
        split_idx = split_idx_lst[0]
    else:
        split_idx = split_idx_lst[run]
    ...
    for epoch in range(args.epochs):
        ...
        # 根据数据集自动选择 loss计算方式
        if args.dataset in MULTI_LABEL_DATASETS:
            loss = criterion(out[train_idx], dataset.label[train_idx].float())
        else:
    ...
    # Save attention weights if required
    if args.output_attention:
        attentions = model.get_attentions(dataset.graph['node_feat'].to(device))
        np.save(f'results/att/{args.dataset}_{args.method}_{args.hidden_channels}_attentions.npy',
                attentions.detach().cpu().numpy())

    # Delete model and optimizer for the next run
    del model, optimizer
