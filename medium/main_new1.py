...
#    criterion = nn.BCEWithLogitsLoss() if args.dataset in MULTI_LABEL else nn.CrossEntropyLoss()
    ...
    if args.vis_emb:
        ...
        model.eval()
        with torch.no_grad():
            if hasattr(model, 'trans_conv'):
                layer_id = args.vis_layer if args.vis_layer >= 0 else -1
                x_emb_src = model.trans_conv.convs[layer_id].fp
            elif hasattr(model, 'feature_layer'):
                x_emb_src = model.feature_layer
            else:
                x_emb_src = dataset.graph["node_feat"]

            if callable(x_emb_src):
                x_feat = dataset.graph["node_feat"]
                x_emb = x_emb_src(x_feat)
            else:
                x_emb = x_emb_src
