...
class PoincareMath:
    """
    Poincaré Ball 模型基础数学运算工具类
    基于文稿 Section 2.2 [cite: 96-113]
    """
    ...
    def dist(self, x, y):
        ...
        # 实际上使用 Möbius addition: ||-x \oplus y||
        neg_x = -x
        diff = self.mobius_add(neg_x, y)
        norm_diff = diff.norm(dim=-1, p=2)
        # 数值稳定性处理
        norm_diff = torch.clamp(norm_diff, max=1.0 / (self.c ** 0.5) - self.eps)
        dist = 2 / (self.c ** 0.5) * torch.atanh(self.c ** 0.5 * norm_diff)
        return dist

    def einstein_midpoint(self, x, weights):
        """
        Weighted Einstein Midpoint (聚合) [cite: 166-176]
        为了数值稳定和效率，通过 Klein 模型进行计算:
        Poincaré -> Klein -> Weighted Mean -> Poincaré
        """
        # 1. Map Poincaré to Klein
        x2 = x.pow(2).sum(dim=-1, keepdim=True)
        gamma = 1 / (1 - self.c * x2 + self.eps)  # Lorentz factor
        x_klein = 2 * gamma * x / (1 + self.c * x2 * gamma)  # Simplified projection usually involves gamma

        # 更简单的做法：直接利用 Einstein midpoint 公式形式
        # sum(w_i * gamma_i * x_i) / sum(w_i * gamma_i)
        # 注意：这里的 x 是 Poincaré 坐标
        gamma_x = 1.0 / (1.0 - self.c * x.pow(2).sum(dim=-1, keepdim=True).clamp(max=1 - self.eps))

        # 加权求和 (N, E, D)
        # weights: (N, E) -> unsqueeze -> (N, E, 1)
        weighted_sum = (weights.unsqueeze(-1) * gamma_x.unsqueeze(1) * x.unsqueeze(1)).sum(dim=0)  # (E, D)
        sum_gamma = (weights.unsqueeze(-1) * gamma_x.unsqueeze(1)).sum(dim=0)  # (E, 1)

        midpoint_klein = weighted_sum / (sum_gamma + self.eps)

        # Klein to Poincaré
        # P = K / (1 + sqrt(1 - c|K|^2))
        k2 = midpoint_klein.pow(2).sum(dim=-1, keepdim=True)
        midpoint_poincare = midpoint_klein / (1 + torch.sqrt(1 - self.c * k2 + self.eps))
...
##############################################################################
#          5) Dysformer, 保留Minkowski维度, 只在最后才解码
##############################################################################
