...
class HConstructor(nn.Module):
    ...
    # 在 HConstructor 类中替换原有的 _heat_wavelet 方法
    def _heat_wavelet(
            self,
            A_norm: torch.Tensor,
            X: torch.Tensor,
        ...
        """
        使用切比雪夫多项式近似实现的双曲图小波变换 (HGWT)。
        """
        # 1. 缩放拉普拉斯矩阵 L_tilde = L - I = -A_norm
        L_tilde = -A_norm
        outs = []

        for s in scales:
            # 2. 计算切比雪夫系数 (基于标量计算，开销极小)
            coeffs = []
            for k in range(order + 1):
                bessel_val = sp.iv(k, s)  # 第一类修正贝塞尔函数
                if k == 0:
                    c_k = math.exp(-s) * bessel_val
                else:
                    c_k = 2.0 * math.exp(-s) * ((-1) ** k) * bessel_val
                coeffs.append(c_k)

            # 3. 切比雪夫多项式递推 T_k(L_tilde) @ X
            T_0 = X
            Y = coeffs[0] * T_0
...
class Dysformer(nn.Module):
    """
    先走 Dysformer 风格空间-频谱动态超图块，再送入 Lorentz Dysformer Transformer。
    """
