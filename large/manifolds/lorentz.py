...
'''class Lorentz(LorentzOri):
    def __init__(self, k=1.0, learnable=True):
        """
...
class Lorentz(LorentzOri):
    def __init__(self, k=1.0, learnable=True, smooth_factor=0.1):
        # 让父类完成 k 的登记
        super().__init__(k=k, learnable=learnable)

        self.smooth_factor = float(smooth_factor)
        # 记录上一轮曲率，用于平滑；这里用 buffer，不会进优化器
        self.register_buffer("prev_k", self.k.detach().clone())

    @torch.no_grad()
    def _smooth_update_k(self):
        # 仅修改 data，避免再触发 PyTorch 的 __setattr__ 检查
        new_k = self.prev_k + self.smooth_factor * (self.k - self.prev_k)
        self.k.data.copy_(new_k)
        self.prev_k.copy_(new_k)
    ...
    def mid_point(self, x: torch.Tensor, w: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if w is not None:
            ...
            ave = x.mean(dim=-2)  # 普通平均
        denom = (-self.inner(ave, ave, keepdim=True)).clamp_min(1e-8).sqrt()
        return self.k.sqrt() * ave / denom

    def forward(self, x):
        self._smooth_update_k()
    ...
    def _check_point_on_manifold(
        ...
        dn = x.size(dim) - 1
        quad_form = -x.narrow(dim, 0, 1).pow(2) + x.narrow(dim, 1, dn).pow(2).sum(dim=dim, keepdim=True)
        ok = torch.allclose(quad_form, -self.k, atol=atol, rtol=rtol)
        return ok, None if ok else f"Minkowski form ≠ {-self.k.item()}"

    def _check_vector_on_tangent(
        self, x: torch.Tensor, u: torch.Tensor, *, atol=1e-5, rtol=1e-5, dim=-1
    ) -> Tuple[bool, Optional[str]]:
        inner_ = math.inner(u, x, dim=dim)
        ok = torch.allclose(inner_, torch.zeros(1, device=x.device), atol=atol, rtol=rtol)
        return ok, None if ok else "Vector not in tangent space"

    def dist(self: Manifold, x: torch.Tensor, y: torch.Tensor, *, keepdim=False, dim=-1) -> torch.Tensor:
        """
        Compute the distance between two points on the manifold.
    ...
    def dist0(self: Manifold, x: torch.Tensor, *, dim=-1, keepdim=False) -> torch.Tensor:
        """
        Compute the distance from the origin to a point on the manifold.
    ...
    def cdist(self: Manifold, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        """
        Compute the pairwise distance between points in x and y.
    ...
    def lorentz_to_klein(self: Manifold, x: torch.Tensor) -> torch.Tensor:
        """
        Convert a point from Lorentz to Klein coordinates.
    ...
    def klein_to_lorentz(self: Manifold, x: torch.Tensor) -> torch.Tensor:
        """
        Convert a point from Klein to Lorentz coordinates.
    ...
    def lorentz_to_poincare(self: Manifold, x: torch.Tensor) -> torch.Tensor:
        """
        Convert a point from Lorentz to Poincare coordinates.
    ...
    def norm(self: Manifold, u: torch.Tensor, *, keepdim=False, dim=-1) -> torch.Tensor:
        """
        Compute the norm of a tangent vector.
    ...
    def egrad2rgrad(self: Manifold, x: torch.Tensor, u: torch.Tensor, *, dim=-1) -> torch.Tensor:
        """
        Convert Euclidean gradient to Riemannian gradient.
    ...
    def projx(self: Manifold, x: torch.Tensor, *, dim=-1) -> torch.Tensor:
        """
        Project a point onto the manifold.
    ...
    def proju(self: Manifold, x: torch.Tensor, v: torch.Tensor, *, dim=-1) -> torch.Tensor:
        """
        Project a tangent vector onto the tangent space at a point.
    ...
    def proju0(self: Manifold, v: torch.Tensor) -> torch.Tensor:
        """
        Project a tangent vector onto the tangent space at the origin.
    ...
    def expmap(self: Manifold, x: torch.Tensor, u: torch.Tensor, *, norm_tan=True, project=True,
               dim=-1) -> torch.Tensor:
        """
        Perform the exponential map to move from a point in the tangent space to the manifold.
    ...
    def expmap0(self: Manifold, u: torch.Tensor, *, project=True, dim=-1) -> torch.Tensor:
        """
        Perform the exponential map from the origin.
    ...
    def logmap(self: Manifold, x: torch.Tensor, y: torch.Tensor, *, dim=-1) -> torch.Tensor:
        """
        Perform the logarithmic map to move from a point on the manifold to the tangent space.
    ...
    def logmap0(self: Manifold, y: torch.Tensor, *, dim=-1) -> torch.Tensor:
        """
        Perform the logarithmic map from the origin.
    ...
    def logmap0back(self: Manifold, x: torch.Tensor, *, dim=-1) -> torch.Tensor:
        """
        Perform the inverse logarithmic map to move from the tangent space to the manifold.
    ...
    def inner(self: Manifold, x: torch.Tensor, u: torch.Tensor, v: Optional[torch.Tensor] = None, *, keepdim=False,
              dim=-1) -> torch.Tensor:
        """
        Compute the inner product of two tangent vectors at a point.
    ...
    def inner0(self: Manifold, v: torch.Tensor, *, keepdim=False, dim=-1) -> torch.Tensor:
        """
        Compute the inner product of a tangent vector at the origin.
    ...
    def cinner(self: Manifold, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        """
        Compute the cross-inner product of two points.
    ...
    def transp(self: Manifold, x: torch.Tensor, y: torch.Tensor, v: torch.Tensor, *, dim=-1) -> torch.Tensor:
        """
        Perform parallel transport of a tangent vector.
    ...
    def transp0(self: Manifold, y: torch.Tensor, u: torch.Tensor, *, dim=-1) -> torch.Tensor:
        """
        Perform parallel transport from the origin.
    ...
    def transp0back(self: Manifold, x: torch.Tensor, u: torch.Tensor, *, dim=-1) -> torch.Tensor:
        """
        Perform inverse parallel transport to the origin.
    ...
    def transp_follow_expmap(self: Manifold, x: torch.Tensor, u: torch.Tensor, v: torch.Tensor, *, dim=-1,
                             project=True) -> torch.Tensor:
        """
        Perform parallel transport following an exponential map.
    ...
    def mobius_add(self: Manifold, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        """
        Perform Mobius addition of two points.
    ...
    def geodesic_unit(self: Manifold, t: torch.Tensor, x: torch.Tensor, u: torch.Tensor, *, dim=-1,
                      project=True) -> torch.Tensor:
        """
        Compute a point on a geodesic given a time parameter.
