...
def load_dataset(data_dir, dataname, sub_dataname=''):
    ...
    if dataname == 'twitch-e':
        ...
        dataset = load_alzheimers_dataset(data_dir, dataname)
    elif dataname == 'node2vec_PPI':
        dataset = load_node2vec_PPI_dataset(data_dir, dataname)
    elif dataname == 'TUDataset':
        dataset = load_PROTEINS_dataset(data_dir, dataname)
    elif dataname == 'pokec':
        dataset = load_pokec_mat(data_dir)
    else:
...
def load_alzheimers_dataset(data_dir,name):
    # 加载数据
    data_file_path = os.path.join(data_dir, f'{name}.xlsx')
    data = pd.read_excel(data_file_path)

    # 特征和标签
    label_col = 'Group'
    labels = torch.tensor(data[label_col].values, dtype=torch.long)

    if use_feats:
        feature_values = data.drop(label_col, axis=1).values
        node_feat = torch.tensor(feature_values, dtype=torch.float)
    else:
        # 如果不使用特征，则创建一个单位特征矩阵
        node_feat = torch.ones((data.shape[0], 1))
    ...
    dataset.train_idx = indices[:num_train]
    dataset.valid_idx = indices[num_train:num_train + num_val]
    dataset.test_idx = indices[num_train + num_val:]
    ...
    return dataset
def load_node2vec_PPI_dataset(data_dir,name):
    # 加载边列表和标签
    edgelist_file_path = os.path.join(data_dir, f'{name}.edgelist')
    labels_file_path = os.path.join(data_dir, f'{name}_labels.txt')
    ...
    if use_feats:
        node_feat = torch.eye(num_nodes)  # 使用单位矩阵作为特征
    else:
        node_feat = torch.ones((num_nodes, 1))

    # 读取标签
    def read_node_labels(filename):
        node_list, labels_list = [], []
        ...
        labels[torch.tensor(node_list)] = torch.tensor(labels_list)
        return labels

    labels = read_node_labels(labels_file_path)
    ...
    return dataset
def load_PROTEINS_dataset(data_dir,name):
    # 加载 PROTEINS 数据集
    dataset = TUDataset(root=data_dir, name=name)

    # 初始化存储邻接矩阵、特征和标签的列表
    adj_list, features_list, labels_list = [], [], []
    max_feature_dim = max(data.x.shape[1] for data in dataset)
    ...
    for data in dataset:
        ...
        if x.shape[1] < max_feature_dim:
            padding = torch.zeros((x.shape[0], max_feature_dim - x.shape[1]))
            x = torch.cat([x, padding], dim=1)

        num_nodes = x.size(0)
        num_nodes_list.append(num_nodes)
        adj = sp.coo_matrix((np.ones(edge_index.size(1)), (edge_index[0].numpy(), edge_index[1].numpy())),
                            shape=(num_nodes, num_nodes))

        adj_list.append(adj.tocsr())
        features_list.append(x.numpy())
        labels_list.append(y.item())

    adj = sp.block_diag(adj_list)
    node_feat = np.vstack(features_list)
    ...
    idx_train, idx_temp = train_test_split(indices, test_size=(val_prop + test_prop), random_state=seed)
    idx_val, idx_test = train_test_split(idx_temp, test_size=test_prop / (val_prop + test_prop), random_state=seed)
    ...
    dataset.graph = {'edge_index': torch.tensor(np.vstack(adj.nonzero()), dtype=torch.long),
