...
class Dysformer(nn.Module):
    ...
    def forward(self, x, edge_index):
        # ① 三路得到隐向量（均为 hidden_channels 维）
        h_trans = self.manifold_out.logmap0(self.trans_conv(x))[..., 1:]  # [N,D]
        h_graph = self.graph_conv(x, edge_index)  # [N,D]
        if self.hconstructor.edge_index is None:
            self.hconstructor.construct_graph(edge_index, x.size(0))
        h_hyper = self.hypergraph_conv(x, self.hconstructor.edge_index)  # [N,D]

        # ② 叠成 [N,3,D]
        h_stack = torch.stack([h_trans, h_graph, h_hyper], dim=1)  # [N,3,D]

        # ③ 计算注意力得分并 softmax
        score = self.fuse_att(h_stack).squeeze(-1)  # [N,3]
        weight = torch.softmax(score, dim=1).unsqueeze(-1)  # [N,3,1]

        # ④ 加权求和
        h_fused = (weight * h_stack).sum(dim=1)  # [N,D]

        # ⑤ 解码
        if self.decoder_type == 'euc':
            logits = self.decode_final(h_fused)  # [N,C]
        else:  # 'hyp'
            # 把融合后的欧氏向量映射回洛伦兹  →  再用 HypCLS
            z0 = torch.zeros_like(h_fused[:, :1])  # time 分量 0
            h_tan = torch.cat([z0, h_fused], dim=-1)  # [N,D+1] in T₀
            h_hyp = self.manifold_out.expmap0(h_tan)
            logits = self.decode_final(h_hyp)
