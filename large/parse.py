...
def parse_method(args, c, d):
    ...
    model = Dysformer(d, args.hidden_channels, c, graph_weight=args.graph_weight, aggregate=args.aggregate,
...
#                      decoder_type=args.decoder_type,k_n=args.k_n,k_e=args.k_e,num_edges=args.num_edges,epochs=args.epochs,adjust_freq=args.adjust_freq,stage=args.stage,chunk_size=args.chunk_size,iters=args.iters,
                     args=args)
...
def parser_add_main_args(parser):
    ...
    parser.add_argument('--save_result', type=int, choices=[0, 1], default=1, help='Save the result of the run (0: False, 1: True)')
    ...
    parser.add_argument('--hyp_lr', type=float, default=0.001, help='Hyperbolic learning rate (default: 0.01)')
    ...
    parser.add_argument('--num_edges', type=int, default=10, help='   ')
    ...
    parser.add_argument('--adjust_freq', type=int, default=10, help='每多少次 forward 调整一次 self.num_edges (0 表示不调)   ')
    parser.add_argument('--chunk_size', type=int, default=5000000, help='   ')
